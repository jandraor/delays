...
of the $SE^1I^jR$ model ($M^{1j}$) to Poisson $D^{1j}$ incidence reports. We 
mean by *parameterisation* the decision of categorising parameters as either 
*unknown* or *assumed*. For unknown parameters, we construct prior 
...
$I^1_0$. Each candidate is coupled with the appropriate likelihood function. 
That is, the Poisson distribution.
...
```{r}
f_A <- plot_R0_by_fitting_model(ll_df, 10, R0_val, lims = c(2, 3),
                                breaks = c(2, 2.5, 3))
f_B <- plot_incidence_prediction_by_fitting_model(ll_list, y_df, 10, M_i)

g <- f_A/f_B + plot_layout(heights = c(2, 5)) + 
  plot_annotation(tag_levels = 'A') & 
  theme(plot.tag     = element_text(size = 8))

ggsave("./plots/Fig_03_Pois_inf.pdf", plot = g, height = 7, width = 5,
       device = cairo_pdf)
```
...
coupled with the Poisson distribution.
...
```{r}
f_B1 <- plot_R0_vs_tau_by_fitting_model(ll_df, 20, c(0,10), c(0, 6))
```

```{r}
f_B2 <- plot_R0_vs_tau(ll_df, 20, c(0, 10), c(0, 6))
...
```{r}
f_A <- plot_R0_by_fitting_model(ll_df, 20, R0_val, lims = c(1, 6))
```
...
ggsave("./plots/Fig_04_Four_unknowns.pdf", plot = g, height = 7, width = 5, 
      device = cairo_pdf)
```
...
```{r}
p1c <- plot_R0_by_fitting_model(ll_df, 20, R0_val, lims = c(2.4, 2.6),
...
```{r}
t1 <- table_coverage(coverage_df)
temp <- t1 |>  gtsave("t1.png", path = "./plots")
```

```{r}
p2c <- png::readPNG("./plots/t1.png", native = TRUE)

g <- (p1c|p2c) +
  plot_annotation(tag_levels = 'A') & 
  theme(plot.tag     = element_text(size = 8))

ggsave("./plots/Fig_05_Three_unknowns_alt.pdf", plot = g, height = 4.5, 
