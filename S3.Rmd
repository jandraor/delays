...
of the $SE^1I^jR$ model ($M^{1j}$) to **overdispersed $D^{1j}$** incidence 
reports. We mean by *parameterisation* the decision of categorising parameters 
as either *unknown* or *assumed*. For unknown parameters, we construct prior 
...
$I^1_0$. Each candidate is coupled with the appropriate likelihood function. In
this case, given that the data is overdispersed, such likelihood function 
corresponds to the Negative Binomial distribution. The selection of this 
function entails the estimation of an additional parameter, the degree of 
concentration ($\phi^{-1}$) around the incidence's expected value.
...
produced the observed incidences. For instance, $D_{11}$ implies that the fitted
incidence stems from an SEIR with an exponential-distributed infectious period.
...
process of plugging a posterior distribution (samples) into an ODE model to
