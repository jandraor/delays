...
of the $SE^1I^jR$ model ($M^{1j}$ & $M^{3j}$) to **Overdispersed $D^{3j}$** 
...
$I^1_0$. Each candidate is coupled with the Negative Binomial distribution. The 
selection of this likelihood function entails the estimation of an additional 
parameter, the degree of concentration ($\phi^{-1}$) around the incidence's 
expected value.
...
fitting model varies (increasing $j$). In contrast, assuming the wrong
...
p1 <- plot_incidence_prediction_by_E_dist(incidence_df, y_df, D_i, D_j, ds)
```

```{r}
map2(list(joint_posterior_df2, joint_posterior_df1), c(1, 3), \(pos_df, i) {
  
  sub_txt <- parse(text = paste0("M^", i, 3, "~'fitted to'", "~D^33"))
...
summary_df <- select(posterior_sample, par_beta, I0, D_i, D_j, M_i, M_j) |> 
  ...
  summarise(mean        = mean(value),
            ...
            .groups = "drop") |> 
  mutate(label_Mi = paste0("'i = ", M_i,"'"),
         name = case_when(name == "I0" ~ "I[0]",
                          name == "R0" ~ "\u211c[0]"))


vline_df <- data.frame(name = c("I[0]", "\u211c[0]"), val = c(1, R0_val))

p3 <- ggplot(summary_df, aes(mean, as.factor(M_j))) +
  ...
  facet_grid(label_Mi~name, scales = "free", labeller = label_parsed) +
  labs(x = "Value", y = "j",
       title = parse(text = paste0("'Error bars:'~M^ij")),
       subtitle = parse(text = paste0("'Vertical line:'~D^33"))) +
  theme_classic() +
  theme(text = element_text(family = "Arial Unicode MS"),
        ...
        axis.ticks    = element_line(colour = "grey60"),
        axis.line     = element_line(colour = "grey80"),
        plot.title    = element_text(colour = "grey10", size = 9,
                                     margin = margin(0,0,0,0, "cm")),
        plot.subtitle = element_text(colour = "grey10", size = 9,
                                     margin = margin(0,0,0,0, "cm")),
...
g <- wrap_plots(p1, p3, p2) + 
  plot_layout(heights = c(1.5, 2, 3.5)) + 
...
# Three-unknown (Alternative) - Normal

In this section, we test the implications of amalgamating the alternative
parameterisation with a **Normal** measurement model (likelihood function) to 
fit overdispersed incidence data. 

```{r}
root_fldr  <- "./Saved_objects/Inference/Synthetic_data/SEIR/Case_4/Three_params_alt_norm"
M_i        <- 1
M_j        <- 1:4
n_params   <- 5

all_files  <- readRDS("./Stan_files/Inference/Three_params_alt/norm/meta_info.rds")
fltr       <- map_lgl(all_files, \(fl_list) fl_list$E_ord == M_i)
info_files <- all_files[fltr]
...
The prior distributions shown below apply for all candidates from this 
parameterisation. Prior distributions for $\rho$, $I^1_0$ and $\Re_0^{-1}$ are 
identical to those described [here](#link3).


### Standard deviation ($\epsilon$)

The choice of a Gaussian measurement model under the assumption of 
homoscedasticity implies the estimation of an additional parameter, the standard
deviation denoted by the Greek letter epsilon ($\epsilon$).

```{r, fig.align='left', fig.height = 3, fig.width = 5}
prior_df <- data.frame(x = rexp(1e5, 0.2))
plot_prior(prior_df, "epsilon", "epsilon~'~ exp(0.2)'", c(0, 30), 15)
```
...
```{r}
joint_posterior_df <- posterior_sample |> filter(M_j == !!D_j) |> 
  select(par_inv_R0, par_rho, I0, epsilon)
...
```{r}
plot_R0_comparison_by_dataset(posterior_subset, R0_val, 1, 4, D_i, D_j)
```

##### Standard deviation ($\epsilon$)

```{r}
prior_df <- data.frame(sims = rexp(4e3, 0.2))
prior_posterior_comparison(posterior_sample, prior_df, "epsilon",
                           "Grey histogram (prior): exp(0.2)")
```
...
```{r}
plot_hist_mase(posterior_sample, y_df, c(0, 2.5))
```
