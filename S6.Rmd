...
parameterisation under various levels of transmission ($\Re$) and mean 
generation times. For simplicity, we restrict the $SE^iI^jR$ to four instances
($i = 1$, and $j = \{1,2,3,4\}$) for data generation and matching.
...
denoted by ($\tau_e$). Nonetheless, we generate synthetic for all four values of
$j$ (20 each). Furthermore, data is generated for two configurations of the 
Negative Binomial measurement model: Poisson ($\phi^{-1} = 0$) & overdispersed
$\phi^{-1} = \frac{1}{3}$. As a result, we produce 160 synthetic incidence
reports per scenario. Then, we fit four candidates per incidence report, where
we assume the appropriate measurement model.
...
plot_meas_incidence_by_overdispersion(y_df)
```
